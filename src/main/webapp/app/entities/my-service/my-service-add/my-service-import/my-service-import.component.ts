...
import { CommonServiceCode } from '../../../common-service-code/common-service-code.model';
...
@Component({
  selector: 'bc-my-service-import',
  templateUrl: './my-service-import.component.html',
})
export class MyServiceImportComponent implements OnInit {
  isSaving = false;
  myServicePageFlow: ServiceEventType = ServiceEventType.GENERAL_ADD;

  @Output()
  eventEmitter = new EventEmitter<CommonServiceCode>();
  ...
  addSelectService(source: CommonServiceCode) {
    ...
    if (this.myService !== undefined && this.myService !== null) {
      ...
      this.eventEmitter.emit(this.selectedCommonServiceCode);
      // this.eventServiceEmitter.emit(this.myService);
    }
  }
  ...
  removeSelectService() {
    ...
    this.selectedCommonServiceCode = {} as CommonServiceCode;
    this.eventEmitter.emit(this.selectedCommonServiceCode);
    if (this.myService !== undefined && this.myService !== null) {
  ...
  isSelected(commonServiceCode: CommonServiceCode): boolean {
    if (commonServiceCode.id === this.myService!.commonServiceCodeId) {
      return true;
    }
    return false;
  }
  ...
  protected onSuccessServiceCode(data: any, headers: HttpHeaders): void {
    ...
    // in search mode selectedSerrvice code not changed
    if (!this.currentSearch) {
      const commonServiceCodes = this.commonServiceCodes || [];
      const selectedServiceCode = commonServiceCodes.find(p => p.id === this.myService!.commonServiceCodeId);
      this.selectedCommonServiceCode = selectedServiceCode;
    }
  }
