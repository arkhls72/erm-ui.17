...
import { InstructionService, RestInstruction } from './instruction.service';

const requireRestSample: RestInstruction = {
...
describe('Instruction Service', () => {
  ...
  let expectedResult: Instruction | Instruction[] | boolean | null;
  ...
  describe('Service methods', () => {
    ...
    describe('addInstructionToCollectionIfMissing', () => {
      it('should add a Instruction to an empty array', () => {
        const instruction: Instruction = sampleWithRequiredData;
      ...
      it('should not add a Instruction to an array that contains it', () => {
        const instruction: Instruction = sampleWithRequiredData;
        const instructionCollection: Instruction[] = [
          {
            ...instruction,
          },
      ...
      it("should add a Instruction to an array that doesn't contain it", () => {
        const instruction: Instruction = sampleWithRequiredData;
        const instructionCollection: Instruction[] = [sampleWithPartialData];
        expectedResult = service.addInstructionToCollectionIfMissing(instructionCollection, instruction);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(instruction);
      });
      ...
      it('should add only unique Instruction to an array', () => {
        const instructionArray: Instruction[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const instructionCollection: Instruction[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const instruction: Instruction = sampleWithRequiredData;
        const instruction2: Instruction = sampleWithPartialData;
