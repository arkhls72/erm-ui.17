...
export class OrderPlacementComponent implements OnInit, OnDestroy {
  ...
  loadProductOrders() {
    this.productOrderService.findByOrderNumber(this.order.id).subscribe(res => {
      const productOrder = res.body;
      this.productOrders = productOrder;
    });
  }
  loadProductByOrder() {
    this.productService.findSummaryProductByOrder(this.order.id).subscribe(res => {
      const products: any = res.body;
      this.products = products;
    });
  }
  deleteRow(i: number) {
    this.selectedProductOrder = this.productOrders !== null ? this.productOrders[i] : null;
  ...
  initItemPrice(id: any) {
    ...
    return foundProduct.orderPrice;
  }
