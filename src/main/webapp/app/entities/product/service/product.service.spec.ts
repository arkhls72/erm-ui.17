...
const requireRestSample: RestProduct = {
  ...
  lastOrderDate: sampleWithRequiredData.lastOrderDate?.toJSON(),
...
describe('Product Service', () => {
  ...
  let expectedResult: Product | Product[] | boolean | null;
  ...
  describe('Service methods', () => {
    ...
    describe('addProductToCollectionIfMissing', () => {
      it('should add a Product to an empty array', () => {
        const product: Product = sampleWithRequiredData;
      ...
      it("should add a Product to an array that doesn't contain it", () => {
        const product: Product = sampleWithRequiredData;
        const productCollection: Product[] = [sampleWithPartialData];
        expectedResult = service.addProductToCollectionIfMissing(productCollection, product);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(product);
      });
      ...
      it('should add only unique Product to an array', () => {
        const productArray: Product[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const productCollection: Product[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const product: Product = sampleWithRequiredData;
        const product2: Product = sampleWithPartialData;
