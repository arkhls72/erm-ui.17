...
@Component({
  standalone: true,
  selector: 'bc-exercise-instruction-add',
  templateUrl: './exercise-instruction-add.component.html',
  imports: [
    ReactiveFormsModule,
    FaIconComponent,
    CommonModule,
...
export class ExerciseInstructionAddComponent implements OnInit {
  editMode = false;
  addMode = false;
  ...
  progExerciseInstruction!: ProgExerciseInstruction;
  ...
  constructor(
    ...
    protected progInstructionService: ProgExerciseInstructionService,
  ...
  ngOnInit(): void {
    this.loadCurrentProgInstruction();
    this.handleNavigation();
  }

  loadCurrentProgInstruction() {
    if (this.selectedProg && this.selectedExercise) {
      this.progInstructionService.findByProgExercise(this.selectedProg.id, this.selectedExercise.id).subscribe(res => {
        if (res && res.body) {
          this.progExerciseInstruction = res.body;
          if (this.progExerciseInstruction && this.progExerciseInstruction.instruction) {
            this.selectedInstruction = this.progExerciseInstruction.instruction;
            this.updateForm(this.progExerciseInstruction.instruction);
          }
        }
      });
    }
  ...
  save(): void {
    ...
    const target =
      this.progExerciseInstruction && this.progExerciseInstruction.id ? this.progExerciseInstruction : ({} as ProgExerciseInstruction);
    target.instruction = instruction;
    target.exerciseId = this.selectedExercise.id;
    target.progId = this.selectedProg.id;

    if (!target.id) {
      this.subscribeToSaveResponse(this.progInstructionService.create(target));
    } else {
      this.subscribeToSaveResponse(this.progInstructionService.update(target));
    }
  }
  ...
  protected onSaveSuccess(data: ProgGroupInstruction | null): void {
    if (data) {
      ...
      this.progExerciseInstruction = data;
      if (data.instruction) {
        ...
        if (!tree) {
          ...
          const cacheProgInstrcution = tree.progExerciseInstructions;
          // cache is empty
          if (!cacheProgInstrcution || cacheProgInstrcution.length === 0) {
            // push the first element in to cache
            tree.progExerciseInstructions = [];
            tree.progExerciseInstructions.push(data);
            this.cacheTree.addTreeExercise(tree);
          } else {
            ...
            tree.progExerciseInstructions = [];
            tree.progExerciseInstructions = cacheProgInstrcution;
            this.cacheTree.addTreeExercise(tree);
          }
        }
      }
  ...
  loadInstruction(page?: number) {
    ...
    this.instructionService
      .findByProgIdExerciseIdPageable(
        {
          page: pageToLoad - 1,
