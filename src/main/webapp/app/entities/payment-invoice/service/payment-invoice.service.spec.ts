...
describe('PaymentInvoice Service', () => {
  ...
  let expectedResult: PaymentInvoice | PaymentInvoice[] | boolean | null;
  ...
  describe('Service methods', () => {
    ...
    describe('addPaymentInvoiceToCollectionIfMissing', () => {
      it('should add a PaymentInvoice to an empty array', () => {
        const paymentInvoice: PaymentInvoice = sampleWithRequiredData;
      ...
      it('should not add a PaymentInvoice to an array that contains it', () => {
        const paymentInvoice: PaymentInvoice = sampleWithRequiredData;
        const paymentInvoiceCollection: PaymentInvoice[] = [
          {
            ...paymentInvoice,
          },
      ...
      it("should add a PaymentInvoice to an array that doesn't contain it", () => {
        const paymentInvoice: PaymentInvoice = sampleWithRequiredData;
        const paymentInvoiceCollection: PaymentInvoice[] = [sampleWithPartialData];
        expectedResult = service.addPaymentInvoiceToCollectionIfMissing(paymentInvoiceCollection, paymentInvoice);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(paymentInvoice);
      });
      ...
      it('should add only unique PaymentInvoice to an array', () => {
        const paymentInvoiceArray: PaymentInvoice[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const paymentInvoiceCollection: PaymentInvoice[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const paymentInvoice: PaymentInvoice = sampleWithRequiredData;
        const paymentInvoice2: PaymentInvoice = sampleWithPartialData;
