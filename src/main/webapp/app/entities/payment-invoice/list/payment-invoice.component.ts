...
export class PaymentInvoiceComponent implements OnInit {
  ...
  trackId = (_index: number, item: PaymentInvoice): number => this.paymentInvoiceService.getPaymentInvoiceIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(paymentInvoice: PaymentInvoice): void {
  ...
  protected fillComponentAttributesFromResponseBody(data: PaymentInvoice[] | null): PaymentInvoice[] {
    return data ?? [];
  }
